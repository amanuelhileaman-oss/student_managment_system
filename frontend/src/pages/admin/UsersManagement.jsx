...
import { resolveFileUrl, getBackendHost, triggerFileDownload, triggerFilePreview } from '../../utils/fileUrl';
...
const UsersManagement = () => {
  ...
  const handleViewFullDocument = async (doc) => {
    if (!doc) return;
    ...
    try {
      ...
      if (docData.startsWith('data:')) {
        ...
        if (win) {
          win.document.write(
            `<iframe src="${docData}" frameborder="0" style="border:0; top:0; left:0; bottom:0; right:0; width:100%; height:100%;" allowfullscreen></iframe>`
          );
        } else {
  ...
  const handleDownloadDocument = async (doc) => {
    ...
    try {
      ...
      if (doc.grade8_document_signed_url) {
        const link = document.createElement('a');
        link.href = doc.grade8_document_signed_url;
        link.download = docName;
        link.target = '_blank';
        document.body.appendChild(link);
        link.click();
        ...
        {selectedDoc && (
          <div className="space-y-4 text-sm max-w-xl">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] px-2 py-0.5 rounded-full font-medium bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                    {selectedDoc.grade8_document_type || 'Official Document'}
                  </span>
                </div>
                {selectedDoc.grade8_document_data && (
                  <button
                    ...
                    className="text-xs text-primary-600 dark:text-primary-400 hover:underline flex items-center gap-1 font-semibold disabled:opacity-50"
                  >
              ...
              {docActionError && (
                <div className="mb-3 p-2.5 rounded-lg bg-rose-50 dark:bg-rose-950/50 border border-rose-200 dark:border-rose-900 text-rose-700 dark:text-rose-300 text-xs flex items-center gap-2">
              ...
              {(() => {
                ...
                return (
                  <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-950 p-4 flex flex-col items-center justify-center">
                    {previewImg ? (
                      <div className="relative group max-h-80 w-full overflow-hidden flex items-center justify-center mb-4">
                        <img
                          src={previewImg}
                          alt="Grade 8 Certificate"
                          className="max-h-80 w-auto object-contain rounded shadow border border-slate-800 cursor-pointer transition-transform hover:scale-[1.01]"
                          onClick={() => handleViewFullDocument(selectedDoc)}
                          onError={(e) => {
                      ...
                      <div className="py-6 text-center mb-2">
                        <FileText className="w-14 h-14 text-amber-500 mx-auto mb-2" />
                        <p className="text-sm font-semibold text-slate-100 mb-1">{docName || 'Grade 8 Completion Document'}</p>
                        <p className="text-xs text-slate-400 uppercase tracking-wider">{ext ? `${ext.toUpperCase()} File` : (docType || 'Official Document')}</p>
                      </div>
                    )}

                    {/* Primary Action Controls */}
                    <div className="flex flex-wrap items-center justify-center gap-3 pt-3 w-full border-t border-slate-800">
                      <button
                        type="button"
                        onClick={() => handleViewFullDocument(selectedDoc)}
                        disabled={docActionLoading}
                        className="px-4 py-2 rounded-xl bg-primary-600 hover:bg-primary-500 active:scale-95 text-white text-xs font-bold inline-flex items-center gap-2 shadow-sm transition-all disabled:opacity-50"
                      >
                        {docActionLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Eye className="w-3.5 h-3.5" />}
                        <span>View Full Official Document</span>
                      </button>
                      <button
                        type="button"
                        onClick={() => handleDownloadDocument(selectedDoc)}
                        disabled={docActionLoading}
                        className="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 active:scale-95 text-white text-xs font-bold inline-flex items-center gap-2 shadow-sm transition-all border border-slate-700 disabled:opacity-50"
                      >
                        <Download className="w-3.5 h-3.5" />
